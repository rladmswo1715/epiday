...
const EpidayForm = ({ epidayId, initialData, isEdit = false }: EpidayFormProps) => {
  ...
  return (
    <div className='bg-var-blue-100'>
      <InnerLayout>
        <section className='pb-[3rem] pt-[2.4rem] sm:pb-[5.2rem] sm:pt-[5.6rem]'>
          {loading && <Spinner />}
          <h2 className='mb-[4rem] text-[1.6rem] font-[600] leading-[2.6rem] sm:text-[2.4rem] sm:leading-[3.2rem]'>에피데이 만들기</h2>
          <form onSubmit={handleSubmit(onSubmit)}>
            <div className='flex flex-col gap-[4rem] sm:gap-[5.4rem]'>
              <ContentTitle contentTitle='내용' fontSizeStyle='sm:text-[2rem] text-[1.4rem]' marginStyle='sm:mb-[2.4rem] mb-[0.8rem]' isRequired>
                <textarea
                  className={`h-[14.8rem] w-[100%] overflow-scroll rounded-[1.2rem] border-[0.1rem] ${errors.content ? 'border-var-error' : 'border-var-blue-300'} px-[1rem] py-[1.6rem] text-[1.6rem] leading-[2.6rem] scrollbar-hide focus:outline-none sm:text-[2rem] sm:leading-[3.2rem]`}
                  maxLength={500}
                  placeholder='500자 이내로 입력해주세요.'
                  {...register('content')}
                />
                <div className='mt-[0.6rem] flex justify-between text-[1.3rem] leading-[2.2rem] sm:text-[1.6rem] sm:leading-[2.6rem]'>
                  <span className='text-var-error'>{errors.content?.message}</span>
                  <span className='text-var-black-300'>{content.length}/500</span>
                </div>
              </ContentTitle>
              <ContentTitle contentTitle='저자' fontSizeStyle='sm:text-[2rem] text-[1.4rem]' marginStyle='sm:mb-[2.4rem] mb-[0.8rem]' isRequired>
                ...
                <TextInput cssStyle='sm:mt-[1.6rem] mt-[1.2rem]' placeholder='저자 이름 입력' maxLength={30} disabled={author === 'unknown' || author === 'self'} {...register('authorName')} />
                <span className='mt-[0.6rem] text-[1.3rem] leading-[2.2rem] text-var-error sm:text-[1.6rem] sm:leading-[2.6rem]'>{errors.authorName?.message}</span>
              </ContentTitle>
              <ContentTitle contentTitle='출처' fontSizeStyle='sm:text-[2rem] text-[1.4rem]' marginStyle='sm:mb-[2.4rem] mb-[0.8rem]'>
                <TextInput placeholder='출처 제목 입력' maxLength={100} {...register('referenceTitle')} />
                <TextInput cssStyle='sm:mt-[1.6rem] mt-[0.8rem]' placeholder='URL (ex. https://www.website.com)' {...register('referenceUrl')} />
                <span className='mt-[0.6rem] text-[1.3rem] leading-[2.2rem] text-var-error sm:text-[1.6rem] sm:leading-[2.6rem]'>{errors.referenceUrl?.message}</span>
              </ContentTitle>
              <ContentTitle contentTitle='태그' fontSizeStyle='sm:text-[2rem] text-[1.4rem]' marginStyle='sm:mb-[2.4rem] mb-[0.8rem]'>
                <TagContent />
              </ContentTitle>
            </div>
            <button
              className={`mt-[2.4rem] h-[4.8rem] w-[100%] rounded-[1.2rem] border-[0.1rem] border-var-blue-200 sm:mt-[4rem] sm:h-[6.4rem] ${isValid ? 'bg-var-black-500' : 'bg-var-blue-300'} text-[1.6rem] font-[600] leading-[2.6rem] text-[#FFF] sm:text-[2rem] sm:leading-[3.2rem]`}
              disabled={!isValid}
            >
              {isEdit ? '수정 완료' : '작성 완료'}
            </button>
